...
class scheduler:
    ...
    def next_proc(self, pid=-1):
        ...
        if pid != -1:
            ...
            return
        for index in range(self.curr_proc + 1, len(self.proc_info)):
            if self.proc_info[index][PROC_STATE] == STATE_READY:
                self.curr_proc = index
                self.move_to_running(STATE_READY)
                return
        for index in range(0, self.curr_proc + 1):
            if self.proc_info[index][PROC_STATE] == STATE_READY:
                self.curr_proc = index
                self.move_to_running(STATE_READY)
                return
    ...
    def run(self):
        ...
        while self.get_num_active() > 0:
            ...
            if io_done == True and self.proc_info[self.curr_proc][PROC_STATE] != STATE_RUNNING:
                self.next_proc()
